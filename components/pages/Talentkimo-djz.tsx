...
function useGlobalStyles() {
    useEffect(() => {
        ...
        if (!document.getElementById("talent-kimo-reset")) {
            ...
            style.textContent = `
                *:focus-visible { outline: 2px solid rgba(255,255,255,0.65) !important; outline-offset: 3px !important; box-shadow: none !important; border-radius: 4px; }
...
function PlatformIcon({ platform, size = 16 }: { platform: string; size?: number }) {
    ...
    if (p === "spotify") return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="10" /><path d="M8 15c2-1 4.5-1.2 7-.4" /><path d="M7 12.5c2.5-1.2 6-1.5 9-.5" /><path d="M6.5 10c3-1.5 7.5-1.8 11-.6" /></svg>
...
interface TalentKimoDjzProps {
    heroImage?: string; name?: string; alias?: string; handle?: string; tagline?: string; bio?: string; location?: string
    stat1?: string; stat1Label?: string; stat2?: string; stat2Label?: string; stat3?: string; stat3Label?: string
    stat4?: string; stat4Label?: string; stat5?: string; stat5Label?: string; stat6?: string; stat6Label?: string
    tiktokFollowers?: string; tiktokDesc?: string; instaFollowers?: string; instaDesc?: string
    spotifyFollowers?: string; spotifyDesc?: string; youtubeFollowers?: string; youtubeDesc?: string
    pillar1Title?: string; pillar1Desc?: string; pillar1Icon?: string
    pillar2Title?: string; pillar2Desc?: string; pillar2Icon?: string
    pillar3Title?: string; pillar3Desc?: string; pillar3Icon?: string
    pillar4Title?: string; pillar4Desc?: string; pillar4Icon?: string
    ascensionText?: string; categories?: string
    collab1?: string; collab1Desc?: string; collab2?: string; collab2Desc?: string
    collab3?: string; collab3Desc?: string; collab4?: string; collab4Desc?: string
    brandsWorked?: string; manifesto?: string; ctaTitle?: string; ctaDesc?: string; ctaUrl?: string
}
...
export default function TalentKimoDjz(props: TalentKimoDjzProps) {
    const {
        ...
        name = "Kimo Djz", alias = "Le Créateur de Sons", handle = "@kimodjz",
        ...
        location = "France",
        stat1 = "1M+", stat1Label = "Abonnés TikTok",
        stat2 = "300K", stat2Label = "Abonnés Instagram",
        stat3 = "500K+", stat3Label = "Streams Spotify",
        stat4 = "15M+", stat4Label = "Vues totales",
        stat5 = "2M+", stat5Label = "Vues en -48h",
        stat6 = "11.5%", stat6Label = "Taux d'engagement",
        tiktokFollowers = "1M+",
        ...
        pillar1Title = "Les Sons Viraux", pillar1Desc = "Kimo crée les bandes-son de TikTok. Ses beats deviennent des trends repris par des milliers de créateurs. 'Roule' est devenu un hymne. Chaque nouveau son est un potentiel viral.", pillar1Icon = "🎵",
        pillar2Title = "Le Beatmaking", pillar2Desc = "Avant d'être un influenceur, Kimo est un musicien. Il compose, produit, mixe — et montre tout le processus en stories et en TikTok. Le making of fascine une audience qui veut comprendre comment naît un hit.", pillar2Icon = "🎹",
        pillar3Title = "La Culture Urbaine", pillar3Desc = "Kimo incarne la nouvelle génération de la culture urbaine française. Mode streetwear, références rap, codes de quartier — son univers visuel et sonore parle directement à la Gen Z et aux jeunes millennials.", pillar3Icon = "🏙️",
        pillar4Title = "L'Artiste-Influenceur", pillar4Desc = "Il est à la croisée des chemins : assez artiste pour être crédible musicalement, assez influenceur pour générer de l'engagement massif. Cette double casquette est rare et précieuse.", pillar4Icon = "🔥",
        ...
        collab1 = "Son sponsorisé / Brand Beat", collab1Desc = "Kimo crée un son original ou un beat autour de votre marque — un format natif qui peut devenir un trend TikTok organique.",
        collab2 = "Placement de produit", collab2Desc = "Intégration naturelle dans ses vidéos TikTok, ses sessions studio, ses clips — votre marque dans l'univers sonore et visuel de Kimo.",
        collab3 = "Showcase & Événement", collab3Desc = "Performance live, DJ set, apparition en soirée de marque. Kimo apporte l'énergie et l'audience.",
        collab4 = "Campagne musicale", collab4Desc = "Création d'un morceau dédié à votre campagne — de la composition au clip, en passant par la promotion virale sur TikTok.",
    ...
    const brandList = brandsWorked.split(",").map((s) => s.trim()).filter(Boolean)

    const socialProfiles = {
        instagram: handle && handle !== "@" ? `https://www.instagram.com/${handle.replace("@", "")}` : undefined,
        tiktok: handle && handle !== "@" ? `https://www.tiktok.com/${handle.startsWith("@") ? handle : `@${handle}`}` : undefined,
        spotify: undefined as string | undefined,
        youtube: undefined as string | undefined,
    }

    const platforms = [
        { name: "TikTok", followers: tiktokFollowers, desc: tiktokDesc, main: true },
        { name: "Spotify", followers: spotifyFollowers, desc: spotifyDesc, main: false },
    ...
    const sameAs = [socialProfiles.instagram, socialProfiles.tiktok, socialProfiles.spotify, socialProfiles.youtube].filter(Boolean)
    ...
    return (
        <article lang="fr" aria-labelledby="talent-heading" style={{ width: "100%", background: "#000", color: "#c8c8c8", fontFamily: BODY, overflowX: "hidden", WebkitFontSmoothing: "antialiased" }}>
            <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "Person", name, alternateName: alias, description: bio, url: pageUrl, address: { "@type": "PostalAddress", addressLocality: location }, sameAs, knowsAbout: catList, memberOf: { "@type": "Organization", name: "Just Impact" } }) }} />
            ...
            <header style={{ minHeight: mobile ? "auto" : "100vh", display: "grid", gridTemplateColumns: heroGridColumns, alignItems: "stretch", position: "relative" }}>
                <div style={{ padding: heroPad, display: "flex", flexDirection: "column", justifyContent: "center", order: 1 }}>
                    ...
                    <motion.ul {...getFadeIn(reduced, 0.9)} aria-label="Présence sur les plateformes" style={{ marginTop: 32, display: "flex", gap: 12, listStyle: "none", padding: 0, marginBottom: 0 }}>
                        {["TikTok", "Spotify", "Instagram", "YouTube"].map((platform) => (
            ...
            <Marquee items={["Kimo Djz", "Roule", "Beatmaker", "TikTok", "Culture Urbaine", "Viralité", "Sons", "Studio"]} speed={40} />
                ...
                <Reveal>
                    <div style={{ textAlign: "center", marginBottom: 48 }}>
                        ...
                        <h2 id="stats-heading" style={{ ...sectionTitleStyle, fontSize: "clamp(28px, 3.5vw, 44px)", margin: 0, letterSpacing: -2 }}>
                            L&apos;audience <span aria-hidden="true" style={{ color: WHITE(0.12) }}>Kimo Djz.</span>
                        </h2>
                    </div>
                </Reveal>
                <Reveal>
                ...
                <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                    <Reveal>
                        ...
                        <h2 id="platforms-heading" style={{ ...sectionTitleStyle, fontSize: "clamp(34px, 4.5vw, 58px)", marginBottom: 48 }}>
                            Là où ça <span aria-hidden="true" style={{ color: WHITE(0.12) }}>résonne.</span>
                        </h2>
                    </Reveal>
                ...
                <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                    <Reveal>
                        ...
                        <h2 id="pillars-heading" style={{ ...sectionTitleStyle, fontSize: "clamp(34px, 4.5vw, 58px)", marginBottom: 48 }}>
                            La vibe <span aria-hidden="true" style={{ color: WHITE(0.12) }}>Kimo.</span>
                        </h2>
                    </Reveal>
            ...
            <div style={{ padding: "40px 0" }}>
                <Marquee items={["Roule", "Beatmaker", "Studio", "Freestyle", "Viralité", "Gen Z", "Streetwear", "Spotify"]} speed={30} />
            </div>
            ...
            <section aria-labelledby="manifesto-heading" style={{ padding: mobile ? "72px 20px" : tablet ? "88px 40px" : "100px 72px", display: "flex", justifyContent: "center" }}>
                <Reveal>
                    <div style={{ maxWidth: 800, textAlign: "center" }}>
                        <h2 id="manifesto-heading" style={labelStyle}>Pourquoi Kimo</h2>
            ...
            <section aria-labelledby="cta-heading" style={{ textAlign: "center", padding: mobile ? "88px 20px" : tablet ? "104px 40px" : "120px 72px", position: "relative" }}>
                ...
                <Reveal>
                    <h2 id="cta-heading" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(44px, 7vw, 100px)", lineHeight: 0.95, marginBottom: 20, letterSpacing: -4, color: "#fff", marginTop: 0 }}>{ctaTitle}</h2>
                </Reveal>
                <Reveal delay={0.08}>
                    <p style={{ fontSize: 15, color: WHITE(0.3), maxWidth: 480, margin: "0 auto 40px", lineHeight: 1.8, fontWeight: 300 }}>{ctaDesc}</p>
                </Reveal>
                <Reveal delay={0.16}>
            ...
            <footer style={{ padding: mobile ? "24px 20px" : "28px 72px", borderTop: `1px solid ${WHITE(0.04)}`, display: "flex", flexDirection: mobile ? "column" : "row", justifyContent: "space-between", alignItems: mobile ? "flex-start" : "center", gap: mobile ? 10 : 20 }}>
                <p style={{ fontSize: 10, color: WHITE(0.12), letterSpacing: 4, margin: 0, fontWeight: 700, fontFamily: DISPLAY }}>JUST IMPACT<span aria-hidden="true" style={{ color: WHITE(0.3) }}>.</span> &copy; {new Date().getFullYear()}</p>
