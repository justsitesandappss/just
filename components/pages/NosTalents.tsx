...
import { Fragment, useEffect, useMemo, useRef, useState, useCallback, useId } from "react"
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ display: "flex", gap: 40, width: "max-content", alignItems: "center", minWidth: 0 }}
      >
        {tripled.map((item, i) => (
          <Fragment key={`${item}-${i}`}>
            <span
...
function Counter({ value, label, delay = 0 }: { value: string; label: string; delay?: number }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: "center", width: "100%", minWidth: 0 }} aria-label={`${sanitizeText(label)} : ${sanitizeText(value)}`}>
      ...
      <p style={{ margin: 0, fontSize: 11, color: COLORS.textMuted, fontWeight: 600, letterSpacing: 3, textTransform: "uppercase", fontFamily: BODY, lineHeight: 1.5, overflowWrap: "anywhere" }}>
        {label}
      </p>
    </div>
  )
...
function PlatformIcon({ platform, size = 14 }: { platform: string; size?: number }) {
  ...
  if (p === "instagram")
    return (
      <svg aria-hidden="true" focusable="false" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
...
function TalentCard({ talent, delay = 0 }: { talent: Talent; delay?: number }) {
  ...
  return (
    <motion.article
      ...
      style={{ borderRadius: 24, overflow: "hidden", position: "relative", background: COLORS.panel, border: `1px solid ${hovered && hasLink ? COLORS.borderStrong : COLORS.border}`, display: "flex", flexDirection: "column", transition: reducedMotion ? "none" : "border-color 0.3s ease", minHeight: "100%", width: "100%", minWidth: 0, justifySelf: "stretch" }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {hasLink && (
        <a href={talent.link} aria-label={`Voir le profil de ${safeName}`} style={{ position: "absolute", inset: 0, zIndex: 5, borderRadius: "inherit" }} />
      )}
      <div style={{ position: "relative", aspectRatio: "1 / 1", overflow: "hidden", background: COLORS.panel, width: "100%", minWidth: 0 }}>
        ...
        >
          {safeImage ? (
            ...
            <div role="img" aria-label={`Illustration indisponible pour ${safeName}`} style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, rgba(255,255,255,0.06), rgba(255,255,255,0.02))", color: COLORS.textLow, fontFamily: DISPLAY, fontWeight: 700, fontSize: 18, textTransform: "uppercase", letterSpacing: 1, textAlign: "center", padding: 20 }}>
              Image indisponible
            </div>
          )}
        </motion.div>
        {safeNumber && (
          <p aria-hidden="true" style={{ position: "absolute", top: 16, left: 18, margin: 0, fontFamily: DISPLAY, fontSize: 28, fontWeight: 800, color: "rgba(255,255,255,0.3)", letterSpacing: -1, zIndex: 3, lineHeight: 1 }}>{safeNumber}</p>
        )}
        {safeViews && (
          <div style={{ position: "absolute", top: 16, right: 16, zIndex: 3, display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 100, background: "rgba(0,0,0,0.66)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.08)", maxWidth: "calc(100% - 32px)" }} aria-label={`${safeViews} vues`}>
            <span aria-hidden="true" style={{ fontSize: 11 }}>🔥</span>
            <span style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: COLORS.accent, whiteSpace: "nowrap" }}>{safeViews} vues</span>
          </div>
        )}
        {hasLink && (
          <motion.div aria-hidden="true" animate={reducedMotion ? { opacity: 1 } : { opacity: hovered ? 1 : 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.3 }} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.22)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 4, pointerEvents: "none", padding: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 100, background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.18)", maxWidth: "100%" }}>
              <span style={{ fontFamily: BODY, fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", color: COLORS.text, whiteSpace: "nowrap" }}>Voir le profil</span>
              <svg aria-hidden="true" focusable="false" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: COLORS.text, flexShrink: 0 }}><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
            </div>
          </motion.div>
        )}
      </div>
      <div style={{ padding: "20px 22px 28px", flex: 1, display: "flex", flexDirection: "column", gap: 14, position: "relative", background: COLORS.panel, zIndex: 3, width: "100%", minWidth: 0 }}>
        <header style={{ minWidth: 0 }}>
          <h3 id={titleId} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 20, color: COLORS.text, letterSpacing: -0.5, lineHeight: 1.1, textTransform: "uppercase", margin: "0 0 4px 0", overflowWrap: "anywhere" }}>{safeName}</h3>
          {safeHandle && <p style={{ fontFamily: BODY, fontSize: 12, color: COLORS.textMuted, fontWeight: 400, margin: 0, overflowWrap: "anywhere" }}>{safeHandle}</p>}
        </header>
        {talent.categories.length > 0 && (
          <ul aria-label={`Catégories de ${safeName}`} style={{ display: "flex", flexWrap: "wrap", gap: 6, listStyle: "none", margin: 0, padding: 0, minWidth: 0 }}>
            {talent.categories.map((cat, i) => (
              <li key={`${cat}-${i}`} style={{ fontFamily: BODY, fontSize: 11, fontWeight: 500, color: COLORS.accent, letterSpacing: 0.3, overflowWrap: "anywhere" }}>{cat}</li>
            ))}
          </ul>
        )}
        {talent.followers.length > 0 && (
          <ul aria-label={`Plateformes et audiences de ${safeName}`} style={{ display: "flex", flexWrap: "wrap", gap: 8, listStyle: "none", margin: 0, padding: 0, minWidth: 0 }}>
            {talent.followers.map((f, i) => {
              ...
              return (
                <li key={`${platform}-${count}-${i}`} style={{ minWidth: 0, maxWidth: "100%" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 8, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.06)", color: COLORS.textSoft, minWidth: 0, maxWidth: "100%" }}>
                    <PlatformIcon platform={platform} size={12} />
                    <span style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, color: COLORS.text, letterSpacing: 0.3, minWidth: 0, overflowWrap: "anywhere" }}>
                      <span style={{ position: "absolute", width: 1, height: 1, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 }}>{platform} </span>
                      {count}
                    </span>
                  </div>
                </li>
        ...
        {safeBio && <p id={descId} style={{ fontFamily: BODY, fontSize: 13, lineHeight: 1.7, color: COLORS.textMuted, fontWeight: 300, margin: 0, marginTop: "auto", overflowWrap: "anywhere" }}>{safeBio}</p>}
        {hasLink && (
          <div aria-hidden="true" style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, opacity: hovered ? 1 : 0.72, transform: reducedMotion || !hovered ? "translateX(0)" : "translateX(4px)", transition: reducedMotion ? "none" : "transform 0.3s ease, opacity 0.3s ease", minWidth: 0 }}>
            <span style={{ fontFamily: BODY, fontSize: 10, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", color: COLORS.textLow }}>Découvrir</span>
            <svg aria-hidden="true" focusable="false" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: COLORS.textLow, flexShrink: 0 }}><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
          </div>
        )}
      </div>
    </motion.article>
...
function FilterPill({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  ...
  return (
    <motion.button
      ...
      animate={{ background: active ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.04)", borderColor: active ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.08)", color: active ? COLORS.text : COLORS.textMuted }}
      transition={reducedMotion ? { duration: 0 } : { duration: 0.25 }}
      style={{ padding: "10px 20px", borderRadius: 100, border: "1px solid", fontFamily: BODY, fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", boxShadow: "none", backgroundClip: "padding-box", whiteSpace: "nowrap", flexShrink: 0 }}
    >
      {label}
    </motion.button>
  )
...
function ContactInputField({ id, label, name, type = "text", autoComplete, inputMode, placeholder, required = false, value, error, hint, onChange }: {
  id: string; label: string; name: string; type?: string; autoComplete?: string; inputMode?: HTMLAttributes<HTMLInputElement>["inputMode"]; placeholder?: string; required?: boolean; value: string; error?: string; hint?: string; onChange: (v: string) => void
}) {
...
export default function NosTalentsPage() {
  ...
  const talents: Talent[] = useMemo(() => [
    { name: "Karim Lipton", handle: "@karimlipton94", imageUrl: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=900&q=80&auto=format&fit=crop", categories: ["Humour", "Voyage", "Lifestyle"], followers: [{ platform: "Instagram", count: "1.4M" }, { platform: "TikTok", count: "650.7K" }, { platform: "YouTube", count: "100K" }], views: "7.4M", bio: "Créateur dynamique et authentique, reconnu pour son sens du divertissement.", number: "01", link: "/talents/karim-lipton" },
    { name: "Riles Freestyle", handle: "@rilesfreestyle", imageUrl: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=900&q=80&auto=format&fit=crop", categories: ["Foot", "Journalisme", "Lifestyle"], followers: [{ platform: "Instagram", count: "200.9K" }, { platform: "TikTok", count: "1.3M" }, { platform: "YouTube", count: "61.9K" }], views: "139.1M", bio: "Pro du foot freestyle. Partenaire PSG, Canal+ et SNCF.", number: "02", link: "/talents/riles-freestyle" },
    { name: "Moumlame", handle: "@moumlame59", imageUrl: "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=900&q=80&auto=format&fit=crop", categories: ["Musique", "Lifestyle"], followers: [{ platform: "Instagram", count: "604.8K" }, { platform: "TikTok", count: "732.7K" }, { platform: "Snapchat", count: "19K" }, { platform: "YouTube", count: "12.9K" }], views: "7.9M", bio: "Figure de la Team Nasdas. Spontanéité et authenticité de la nouvelle scène digitale.", number: "03", link: "/talents/moumlame" },
    { name: "Romain Benn", handle: "@romain.benn", imageUrl: "https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?w=900&q=80&auto=format&fit=crop", categories: ["Fitness", "Lifestyle"], followers: [{ platform: "Instagram", count: "144.2K" }, { platform: "TikTok", count: "703.1K" }, { platform: "YouTube", count: "185K" }], views: "4.3M", bio: "Vainqueur The Circle France. Ambassadeur Lifestyle & Fitness.", number: "04", link: "/talents/romain-benn" },
    { name: "Kimo DJZ", handle: "@kimo_djz", imageUrl: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=900&q=80&auto=format&fit=crop", categories: ["Voyage", "Caméras Cachées", "Lifestyle"], followers: [{ platform: "Instagram", count: "170.8K" }, { platform: "TikTok", count: "86.3K" }, { platform: "YouTube", count: "22.7K" }], views: "3.1M", bio: "Bac+6 en marketing digital. Caméras cachées, voyage et actions humanitaires.", number: "05", link: "/talents/kimo-djz" },
    { name: "Inès HMZ", handle: "@ines_hmz04", imageUrl: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=900&q=80&auto=format&fit=crop", categories: ["Voyage", "Lifestyle"], followers: [{ platform: "Instagram", count: "160K" }, { platform: "TikTok", count: "34.6K" }, { platform: "YouTube", count: "14.6K" }], views: "4.2M", bio: "Figure montante du lifestyle et du voyage.", number: "06", link: "/talents/ines-hmz" },
    { name: "Naoil Kohlanta", handle: "@naoil_kohlanta2k20", imageUrl: "https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?w=900&q=80&auto=format&fit=crop", categories: ["Voyage", "Boxe", "Lifestyle"], followers: [{ platform: "Instagram", count: "1.7K" }, { platform: "TikTok", count: "95.5K" }, { platform: "YouTube", count: "82.8K" }], views: "1.6M", bio: "Gagnante Koh-Lanta. Ancienne boxeuse pro, inspiration quotidienne.", number: "07", link: "/talents/naoil-kohlanta" },
    { name: "D.Chinois 93", handle: "@d.chinois93", imageUrl: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=900&q=80&auto=format&fit=crop", categories: ["Humour", "Lifestyle"], followers: [{ platform: "Instagram", count: "150K" }, { platform: "TikTok", count: "3.1K" }], views: "79.5K", bio: "Humour spontané et style direct qui résonne avec une audience jeune.", number: "08", link: "/talents/d-chinois-93" },
    { name: "Just Mini", handle: "@justminioff", imageUrl: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=900&q=80&auto=format&fit=crop", categories: ["Boxe", "Lifestyle"], followers: [{ platform: "Instagram", count: "40K" }, { platform: "TikTok", count: "19.4K" }, { platform: "YouTube", count: "38.7K" }], views: "3.2M", bio: "Créateur dynamique et boxeur engagé. Performance sportive et authenticité.", number: "09", link: "/talents/just-mini" },
  ], [])
  ...
  const filteredTalents = useMemo(() => activeFilter === "Tous" ? talents : talents.filter((t) => t.categories.includes(activeFilter)), [activeFilter, talents])
  ...
  return (
    <div style={{ width: "100%", minWidth: 0, background: COLORS.bg, color: COLORS.textSoft, fontFamily: BODY, overflowX: "hidden", WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@300;400;500;600;700;800&family=Outfit:wght@200;300;400;500;600;700&display=swap');
        input::placeholder, textarea::placeholder { color: rgba(255,255,255,0.28); }
        input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus, textarea:-webkit-autofill { -webkit-box-shadow: 0 0 0px 1000px #000 inset !important; -webkit-text-fill-color: #fff !important; caret-color: #fff !important; }
        a:focus-visible, button:focus-visible { outline: 2px solid ${COLORS.focus}; outline-offset: 3px; box-shadow: 0 0 0 4px rgba(255,255,255,0.14); }
      `}</style>

      <main id="main-content" aria-label="Présentation des talents" style={{ width: "100%", minWidth: 0 }}>
        <section aria-labelledby="main-title" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: sectionPadding, position: "relative", width: "100%", minWidth: 0 }}>
          ...
          <motion.p initial={reducedMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 1, delay: 0.2, ease: EASE }} style={{ fontSize: 11, fontWeight: 600, letterSpacing: 5, textTransform: "uppercase", color: COLORS.textMuted, margin: "0 0 48px 0", display: "flex", alignItems: "center", gap: 12, position: "relative", zIndex: 1, lineHeight: 1.5 }}>
          ...
          <div style={{ width: "100%", maxWidth: 1100, position: "relative", zIndex: 1, minWidth: 0 }}>
            <motion.h1 id="main-title" initial={reducedMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 1, delay: 0.4, ease: EASE }} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(44px, 9vw, 130px)", lineHeight: 0.95, color: COLORS.text, margin: 0, letterSpacing: -4, width: "100%", minWidth: 0 }}>
              <span style={{ display: "block" }}>Nos talents</span>
              <span style={{ display: "block", fontWeight: 300, color: "rgba(255,255,255,0.30)", fontStyle: "italic" }}>font la différence.</span>
            </motion.h1>
          </div>
          <motion.p initial={reducedMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.8, delay: 0.8, ease: EASE }} style={{ marginTop: 40, marginBottom: 0, fontSize: 16, lineHeight: 1.9, maxWidth: 560, color: COLORS.textMuted, fontWeight: 300, position: "relative", zIndex: 1, overflowWrap: "anywhere" }}>
          ...
          <motion.div initial={reducedMotion ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.8, delay: 1.2, ease: EASE }} aria-hidden="true" style={{ position: "absolute", bottom: 40, left: isMobile ? 20 : isTablet ? 40 : 72, display: isMobile ? "none" : "flex", alignItems: "center", gap: 12 }}>
        ...
        <Marquee items={["Influenceurs", "Artistes", "Créateurs", "Talents", "Authenticité", "Engagement", "Impact", "Communauté"]} speed={40} />
        ...
        <section aria-labelledby="stats-title" style={{ padding: sectionPadding, position: "relative" }}>
          ...
          <Reveal>
            <div style={{ maxWidth: 1200, margin: "0 auto" }}>
              <h2 id="stats-title" style={{ position: "absolute", width: 1, height: 1, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 }}>Chiffres clés</h2>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "minmax(0,1fr)" : isTablet ? "repeat(2,minmax(0,1fr))" : "repeat(4,minmax(0,1fr))", gap: isMobile ? 24 : 48, width: "100%", minWidth: 0 }}>
        ...
        <section aria-labelledby="roster-title" style={{ padding: isMobile ? "40px 20px 80px" : isTablet ? "40px 40px 80px" : "40px 72px 100px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <Reveal>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 5, textTransform: "uppercase", color: COLORS.textLow, margin: "0 0 14px 0" }}>Notre roster</p>
              <h2 id="roster-title" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.5vw, 58px)", color: COLORS.text, lineHeight: 1, letterSpacing: -3, margin: "0 0 36px 0", overflowWrap: "anywhere" }}>
                Nos influenceurs <span style={{ color: "rgba(255,255,255,0.30)" }}>& artistes.</span>
              </h2>
            </Reveal>
            <Reveal delay={0.05}>
              <div role="group" aria-label="Filtres des talents" aria-describedby={resultCountId} style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 48, width: "100%", minWidth: 0 }}>
                {filterOptions.map((cat) => <FilterPill key={cat} label={cat} active={activeFilter === cat} onClick={() => setActiveFilter(cat)} />)}
              </div>
            </Reveal>
            <p id={resultCountId} aria-live="polite" style={{ position: "absolute", width: 1, height: 1, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 }}>
              {filteredTalents.length} talent{filteredTalents.length > 1 ? "s" : ""} affiché{filteredTalents.length > 1 ? "s" : ""} pour le filtre {activeFilter}.
            </p>
            <motion.div layout={!reducedMotion} style={{ display: "grid", gridTemplateColumns: isMobile ? "minmax(0,1fr)" : isTablet ? "repeat(2,minmax(0,1fr))" : "repeat(3,minmax(0,1fr))", gap: 16, width: "100%", minWidth: 0, alignItems: "stretch" }}>
        ...
        <section aria-labelledby="why-title" style={{ padding: isMobile ? "60px 20px" : isTablet ? "60px 40px" : "80px 72px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <Reveal>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: 5, textTransform: "uppercase", color: COLORS.textLow, margin: "0 0 14px 0" }}>Pourquoi nos talents</p>
              <h2 id="why-title" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.5vw, 58px)", color: COLORS.text, lineHeight: 1, letterSpacing: -3, margin: "0 0 48px 0", overflowWrap: "anywhere" }}>
                Ce qui les rend <span style={{ color: "rgba(255,255,255,0.30)" }}>uniques.</span>
              </h2>
            </Reveal>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "minmax(0,1fr)" : isTablet ? "repeat(2,minmax(0,1fr))" : "repeat(3,minmax(0,1fr))", gap: 12, width: "100%", minWidth: 0, alignItems: "stretch" }}>
              {[
                { n: "01", t: "Audiences vérifiées", d: "Chaque créateur est audité : vrais followers, vrais engagements. Zéro bot, zéro fake." },
                { n: "02", t: "Alignement de marque", d: "On associe les talents aux marques qui leur ressemblent. Pas de forcing, seulement de l'authenticité." },
                { n: "03", t: "Résultats mesurables", d: "KPIs clairs, reporting en temps réel, ROI traçable." },
                { n: "04", t: "Exclusivité sectorielle", d: "Nos talents s'engagent sur des exclusivités par secteur pour protéger votre partenariat." },
                { n: "05", t: "Production intégrée", d: "Grâce à Just Prod, nous produisons du contenu premium directement avec nos talents." },
                { n: "06", t: "Management 360°", d: "Négociation, planning, brief créatif, validation, nous gérons la relation talent de A à Z." },
              ].map((item, index) => (
                <Reveal key={item.n} delay={index * 0.04}>
                  <article style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: "40px 28px", position: "relative", overflow: "hidden", minHeight: "100%", width: "100%", minWidth: 0 }}>
                    <p aria-hidden="true" style={{ margin: 0, fontFamily: DISPLAY, fontSize: 64, fontWeight: 800, color: "rgba(255,255,255,0.05)", lineHeight: 1, letterSpacing: -3, position: "absolute", top: -4, right: 16 }}>{item.n}</p>
                    <h3 style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 18, color: COLORS.text, letterSpacing: -0.5, margin: "0 0 10px 0", lineHeight: 1.2, position: "relative", zIndex: 1, overflowWrap: "anywhere" }}>{item.t}</h3>
                    <p style={{ fontFamily: BODY, fontSize: 14, lineHeight: 1.8, color: COLORS.textMuted, fontWeight: 300, margin: 0, position: "relative", zIndex: 1, overflowWrap: "anywhere" }}>{item.d}</p>
                  </article>
                </Reveal>
              ))}
            </div>
        ...
        <Marquee items={["Humour", "Sport", "Musique", "Fitness", "Voyage", "Lifestyle", "Boxe", "Foot"]} speed={30} />

        <section aria-label="Formulaire de contact" style={{ padding: isMobile ? "80px 20px 100px" : isTablet ? "100px 40px 120px" : "110px 72px 140px", borderTop: `1px solid ${white(0.06)}`, position: "relative" }}>
            ...
            <Reveal>
              <div style={{ marginBottom: isMobile ? 48 : 72 }}>
                ...
                <h2 style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: isMobile ? "clamp(40px, 12vw, 58px)" : "clamp(56px, 8vw, 110px)", lineHeight: 0.92, color: "#ffffff", margin: 0, letterSpacing: isMobile ? -2 : -5 }}>
                  ...
                  <span style={{ display: "block", fontWeight: 300, fontStyle: "italic", color: white(0.88), letterSpacing: isMobile ? -1 : -3 }}>de votre projet.</span>
                </h2>
                ...
                <div>
                  {status === "success" ? (
                    ...
                    <section>
                      ...
                      <p style={{ margin: "0 0 52px", fontFamily: BODY, fontSize: 15, lineHeight: 1.8, color: white(OP.desc), maxWidth: 620 }}>Les champs marqués d'un astérisque sont obligatoires.</p>
              ...
              <Reveal delay={0.15}>
                <aside aria-label="Informations de contact" style={{ position: isMobile ? "relative" : "sticky", top: 100, display: "flex", flexDirection: "column", gap: 24 }}>
                  ...
                  <div style={{ padding: isMobile ? "24px" : "28px", borderRadius: 20, background: white(0.02), border: `1px solid ${white(0.08)}` }}>
                    ...
                    <blockquote style={{ margin: 0, fontFamily: DISPLAY, fontSize: isMobile ? 22 : 28, fontWeight: 300, lineHeight: 1.5, color: white(OP.quote), fontStyle: "italic", letterSpacing: -0.8 }}>
