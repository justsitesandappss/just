...
function loadFonts() {
    ...
    l.rel = "stylesheet"
    l.href = "https://fonts.googleapis.com/css2?family=Syne:wght@300;400;500;600;700;800&family=Outfit:wght@200;300;400;500;600;700&family=Cormorant+Garamond:ital,wght@1,200;1,300&family=DM+Mono:wght@300;400&display=swap"
    document.head.appendChild(l)
    if (!document.getElementById("just-merged-numeric-fonts")) {
        const l2 = document.createElement("link")
        l2.id = "just-merged-numeric-fonts"
        l2.rel = "stylesheet"
        l2.href = "https://api.fontshare.com/v2/css?f[]=satoshi@700,800,900&display=swap"
...
function getMotionProps(reducedMotion: boolean | null, inView = true, initialY = 30, duration = 0.7, delay = 0) {
    if (reducedMotion) {
...
function PlatformIcon({ platform, size = 14 }: { platform: string; size?: number }) {
    ...
    if (p === "instagram") return <svg aria-hidden="true" focusable="false" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><rect x="2" y="2" width="20" height="20" rx="5" /><circle cx="12" cy="12" r="5" /><circle cx="17.5" cy="6.5" r="1.5" fill={color} stroke="none" /></svg>
    if (p === "tiktok") return <svg aria-hidden="true" focusable="false" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5" /></svg>
    ...
    if (p === "snapchat") return <svg aria-hidden="true" focusable="false" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M12 2C8 2 6 5 6 8v2c-1 0-2 .5-2 1s1 1 2 1c-.5 2-2 3-4 4 0 .5.5 1 2 1 0 .5 0 1 .5 1.5S6 19 8 19.5c0 1 .5 2.5 4 2.5s4-1.5 4-2.5c2-.5 3-1 3.5-1.5s.5-1 .5-1.5c1.5 0 2-.5 2-1-2-1-3.5-2-4-4 1 0 2-.5 2-1s-1-1-2-1V8c0-3-2-6-6-6z" /></svg>
    ...
    return (
        <motion.article ref={ref} {...getMotionProps(reducedMotion, inView, 40, 0.7, delay)} aria-labelledby={titleId} aria-describedby={safeBio ? descId : undefined} style={{ borderRadius: 24, overflow: "hidden", position: "relative", background: COLORS.panel, border: `1px solid ${hovered && hasLink ? COLORS.borderStrong : COLORS.border}`, display: "flex", flexDirection: "column", transition: reducedMotion ? "none" : "border-color 0.3s ease", minHeight: "100%", width: "100%", minWidth: 0, justifySelf: "stretch" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            ...
            <div style={{ position: "relative", aspectRatio: "1 / 1", overflow: "hidden", background: COLORS.panel, width: "100%", minWidth: 0 }}>
                ...
                {safeNumber && (<p className="jm-card-number" aria-hidden="true" style={{ position: "absolute", top: 16, left: 18, margin: 0, fontFamily: DISPLAY, fontSize: 28, fontWeight: 800, color: "rgba(255,255,255,0.3)", letterSpacing: -1, zIndex: 3, lineHeight: 1 }}>{safeNumber}</p>)}

                {safeViews && (
                    <div className="jm-card-views" style={{ position: "absolute", top: 16, right: 16, zIndex: 3, display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 100, background: "rgba(0,0,0,0.66)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.08)", maxWidth: "calc(100% - 32px)" }} aria-label={`${safeViews} vues`}>
            ...
            <div className="jm-card-body" style={{ padding: "20px 22px 28px", flex: 1, display: "flex", flexDirection: "column", gap: 14, position: "relative", background: COLORS.panel, zIndex: 3, width: "100%", minWidth: 0 }}>
                ...
                {filteredCategories.length > 0 && (
                    <ul aria-label={`Catégories de ${safeName}`} style={{ display: "flex", flexWrap: "wrap", gap: 6, listStyle: "none", margin: 0, padding: 0, minWidth: 0 }}>
                        {filteredCategories.map((cat, i) => (<li key={`${cat}-${i}`} style={{ fontFamily: BODY, fontSize: 11, fontWeight: 500, color: COLORS.accent, letterSpacing: 0.3, overflowWrap: "anywhere" }}>{cat}</li>))}
                    </ul>
                )}
                                ...
                                <li key={`${platform}-${count}-${i}`} style={{ minWidth: 0, maxWidth: "100%" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 8, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.06)", color: COLORS.textSoft, minWidth: 0, maxWidth: "100%" }}>
                                        ...
                                        <span style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, color: COLORS.text, letterSpacing: 0.3, minWidth: 0, overflowWrap: "anywhere" }}>
                                            <span className="sr-only">{platform} </span>{count}
                                        </span>
                                    </div>
                                </li>
                            )
...
function FilterPill({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
    const reducedMotion = useReducedMotion()
    return (
        <motion.button type="button" onClick={onClick} aria-pressed={active} className="jm-filter-pill" whileHover={reducedMotion ? undefined : { scale: 1.04 }} whileTap={reducedMotion ? undefined : { scale: 0.97 }} animate={{ background: active ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.04)", borderColor: active ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.08)", color: active ? COLORS.text : COLORS.textMuted }} transition={reducedMotion ? { duration: 0 } : { duration: 0.25 }} style={{ padding: "10px 20px", borderRadius: 100, border: "1px solid", fontFamily: BODY, fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", boxShadow: "none", backgroundClip: "padding-box", whiteSpace: "nowrap", flexShrink: 0 }}>
            {label}
        </motion.button>
    )
}
...
function ContactInputField({ id, label, name, type = "text", autoComplete, inputMode, placeholder, required = false, value, error, hint, onChange }: { id: string; label: string; name: string; type?: string; autoComplete?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"]; placeholder?: string; required?: boolean; value: string; error?: string; hint?: string; onChange: (v: string) => void }) {
    ...
    return (
        <div style={{ position: "relative" }}>
            ...
            <input id={id} name={name} type={type} value={value} required={required} autoComplete={autoComplete} aria-required={required || undefined} aria-invalid={error ? true : undefined} aria-describedby={[hint ? hintId : null, error ? errorId : null].filter(Boolean).join(" ") || undefined} placeholder={focused ? placeholder : ""} onChange={(e) => onChange(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${error ? JC.error : focused ? JC.borderFocus : JC.border}`, padding: "30px 0 16px", fontFamily: BODY, fontSize: 17, fontWeight: 400, color: JC.text, outline: "none", transition: "border-color 0.25s ease", letterSpacing: 0.2 }} />
            {hint ? <p id={hintId} style={{ margin: "10px 0 0", fontFamily: BODY, fontSize: 12, lineHeight: 1.6, color: JC.muted }}>{hint}</p> : null}
            {error ? <p id={errorId} role="alert" style={{ margin: "10px 0 0", fontFamily: BODY, fontSize: 13, lineHeight: 1.6, color: JC.error }}>{error}</p> : null}
        </div>
    )
}

function ContactTextareaField({ id, label, name, placeholder, required = false, value, error, hint, onChange }: { id: string; label: string; name: string; placeholder?: string; required?: boolean; value: string; error?: string; hint?: string; onChange: (v: string) => void }) {
    const [focused, setFocused] = useState(false)
    const active = focused || value.length > 0
    const errorId = cxErrorId(id)
    const hintId = cxHintId(id)
    return (
        <div style={{ position: "relative" }}>
            ...
            <textarea id={id} name={name} value={value} required={required} aria-required={required || undefined} aria-invalid={error ? true : undefined} aria-describedby={[hint ? hintId : null, error ? errorId : null].filter(Boolean).join(" ") || undefined} placeholder={focused ? placeholder : ""} onChange={(e) => onChange(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} rows={5} style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${error ? JC.error : focused ? JC.borderFocus : JC.border}`, padding: "30px 0 16px", fontFamily: BODY, fontSize: 17, fontWeight: 400, color: JC.text, outline: "none", resize: "vertical", minHeight: 130, transition: "border-color 0.25s ease", letterSpacing: 0.2 }} />
            {hint ? <p id={hintId} style={{ margin: "10px 0 0", fontFamily: BODY, fontSize: 12, lineHeight: 1.6, color: JC.muted }}>{hint}</p> : null}
            {error ? <p id={errorId} role="alert" style={{ margin: "10px 0 0", fontFamily: BODY, fontSize: 13, lineHeight: 1.6, color: JC.error }}>{error}</p> : null}
        </div>
    )
...
function parseTalent(name: string, handle: string, imageUrl: string, cats: string, followersStr: string, views: string, bio: string, number: string, link: string): Talent {
    const categories = sanitizeText(cats).split(",").map((s) => s.trim()).filter(Boolean)
    const followers = sanitizeText(followersStr).split(",").map((s) => s.trim()).filter(Boolean).map((entry) => {
...
export default function NosTalents() {
    ...
    const p = {
        ...
        formLabelName: "Nom complet", formLabelEmail: "Adresse email", formLabelPhone: "Téléphone",
        formLabelCompany: "Entreprise / Marque", formLabelMessage: "Votre message", formLabelEntity: "Entité",
        formEntityOptions: "Just Impact, Just Prod, Just Agency, Just 4 You",
        formSuccessTitle: "Message envoyé.", formSuccessDesc: "Merci pour votre intérêt. Notre équipe revient vers vous sous 24h.",
        formErrorTitle: "Une erreur est survenue.", formErrorDesc: "Veuillez réessayer ou nous contacter directement.",
    ...
    const contactEntityList = useMemo(() => p.formEntityOptions.split(",").map((s: string) => s.trim()).filter(Boolean), [p.formEntityOptions])
    const contactNameId = useId(); const contactEmailId = useId(); const contactPhoneId = useId()
    const contactCompanyId = useId(); const contactMessageId = useId(); const contactSectionTitleId = useId()
    const contactSectionDescId = useId(); const contactSuccessTitleId = useId()
    ...
    const handleContactSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            fd.append("from_name", contactFormData.name); fd.append("name", contactFormData.name)
            fd.append("email", contactFormData.email); fd.append("phone", contactFormData.phone)
            fd.append("company", contactFormData.company); fd.append("entity", contactFormData.entity)
            fd.append("message", contactFormData.message)
            const res = await fetch("https://api.web3forms.com/submit", { method: "POST", body: fd })
            const data = await res.json()
            if (res.ok && data?.success) { setContactStatus("success"); setContactLiveMessage(p.formSuccessTitle) }
            else { setContactStatus("error"); setContactLiveMessage(data?.message || p.formErrorTitle) }
        } catch { setContactStatus("error"); setContactLiveMessage(p.formErrorTitle) }
                ...
                <section className="jm-section" aria-labelledby="jm-stats-title" style={{ padding: "100px 72px", position: "relative" }}>
                    ...
                    <Reveal>
                        <div className="jm-section-inner">
                            <h2 id="jm-stats-title" className="sr-only" style={{ margin: 0 }}>Chiffres clés</h2>
                            <div className="jm-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 48, width: "100%", minWidth: 0 }}>
                                <Counter value={p.stat1} label={p.stat1Label} delay={0} />
                                <Counter value={p.stat2} label={p.stat2Label} delay={0.1} />
                                <Counter value={p.stat3} label={p.stat3Label} delay={0.2} />
                                <Counter value={p.stat4} label={p.stat4Label} delay={0.3} />
                            </div>
                        </div>
                    </Reveal>
                </section>
                ...
                <section className="jm-section" aria-labelledby="jm-roster-title" style={{ padding: "40px 72px 100px" }}>
                    <div className="jm-section-inner">
                        <Reveal>
                            ...
                            <h2 id="jm-roster-title" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.5vw, 58px)", color: COLORS.text, lineHeight: 1, letterSpacing: -3, margin: "0 0 36px 0", overflowWrap: "anywhere" }}>
                                Nos influenceurs <span style={{ color: "rgba(255,255,255,0.30)" }}>& artistes.</span>
                            </h2>
                        </Reveal>
                        <Reveal delay={0.05}>
                            <div className="jm-filter-wrap" role="group" aria-label="Filtres des talents" aria-describedby={resultCountId} style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 48, width: "100%", minWidth: 0 }}>
                ...
                <section className="jm-section" aria-labelledby="jm-why-title" style={{ padding: "80px 72px" }}>
                    <div className="jm-section-inner">
                        <Reveal>
                            ...
                            <h2 id="jm-why-title" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.5vw, 58px)", color: COLORS.text, lineHeight: 1, letterSpacing: -3, margin: "0 0 48px 0", overflowWrap: "anywhere" }}>
                                Ce qui les rend <span style={{ color: "rgba(255,255,255,0.30)" }}>uniques.</span>
                            </h2>
                        </Reveal>
                        <div className="jm-values-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12, width: "100%", minWidth: 0, alignItems: "stretch" }}>
                            {[
                        ...
                        <Reveal>
                            <div style={{ marginBottom: isMobile ? 48 : 72 }}>
                                ...
                                <h2 style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: isMobile ? "clamp(40px, 12vw, 58px)" : "clamp(56px, 8vw, 110px)", lineHeight: 0.92, color: "#ffffff", margin: 0, letterSpacing: isMobile ? -2 : -5 }}>
                                    <span style={{ display: "block" }}>{p.formHeroTitle1}</span>
                                    <span style={{ display: "block", fontWeight: 300, fontStyle: "italic", color: white(0.88), letterSpacing: isMobile ? -1 : -3 }}>{p.formHeroTitle2}</span>
                                </h2>
                                ...
                                <div>
                                    {contactStatus === "success" ? (
                                        <motion.section aria-labelledby={contactSuccessTitleId} initial={reducedMotion ? false : { opacity: 0, y: 24 }} animate={reducedMotion ? undefined : { opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: EASE }} style={{ textAlign: "center", padding: isMobile ? "56px 24px" : "80px 40px", background: JC.surface, border: `1px solid ${JC.border}`, borderRadius: 24 }}>
                                            ...
                                            <h3 id={contactSuccessTitleId} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 34, color: "#ffffff", letterSpacing: -2, margin: "0 0 12px" }}>{p.formSuccessTitle}</h3>
                                            <p style={{ fontFamily: BODY, fontSize: 16, color: white(OP.desc), fontWeight: 300, lineHeight: 1.8, maxWidth: 420, margin: "0 auto" }}>{p.formSuccessDesc}</p>
                                            <motion.button type="button" onClick={resetContactForm} whileHover={reducedMotion ? undefined : { opacity: 0.85 }} style={{ marginTop: 36, padding: "14px 30px", borderRadius: 100, border: "1px solid #ffffff", background: "transparent", color: "#ffffff", fontFamily: BODY, fontSize: 12, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer" }}>Nouveau message</motion.button>
                                        </motion.section>
                                    ) : (
                                        <section>
                                            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 6, textTransform: "uppercase", color: white(OP.tag), margin: "0 0 14px" }}>{p.formEyebrow}</p>
                                            <h3 id={contactSectionTitleId} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(32px, 4vw, 48px)", color: "#ffffff", lineHeight: 1, letterSpacing: -2, margin: "0 0 18px" }}>{p.formTitle}</h3>
                                            <p id={contactSectionDescId} style={{ margin: "0 0 52px", fontFamily: BODY, fontSize: 15, lineHeight: 1.8, color: white(OP.desc), maxWidth: 620 }}>{p.formDesc}</p>
                                            <form onSubmit={handleContactSubmit} noValidate>
                                                <div style={{ display: "flex", flexDirection: "column", gap: 36 }}>
                                                    ...
                                                    <ContactTextareaField id={contactMessageId} label={p.formLabelMessage} name="message" required value={contactFormData.message} error={contactErrors.message} placeholder="Décrivez votre projet, vos objectifs, vos délais..." onChange={updateContactField("message")} />
                                                    <AnimatePresence>
                                                        {contactStatus === "error" ? (
                                                            <motion.div role="alert" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} style={{ padding: "16px 22px", borderRadius: 12, background: JC.errorBg, border: `1px solid ${JC.errorBorder}`, fontFamily: BODY, fontSize: 14, color: JC.error, fontWeight: 400, lineHeight: 1.7 }}>
                                                                <strong>{p.formErrorTitle}</strong> {contactLiveMessage && contactLiveMessage !== p.formErrorTitle ? contactLiveMessage : p.formErrorDesc}
                                                            </motion.div>
                                                        ) : null}
                                                    </AnimatePresence>
                            ...
                            <Reveal delay={0.15}>
                                <aside className="jm2-contact-sticky" aria-label="Informations de contact" style={{ position: "sticky", top: 100, display: "flex", flexDirection: "column", gap: 24 }}>
                                    <div style={{ padding: "20px", borderRadius: 16, background: JC.surface, border: `1px solid ${JC.border}`, display: "flex", alignItems: "center", gap: 14 }}>
                                        ...
                                        <p style={{ margin: 0, fontFamily: BODY, fontSize: 13, fontWeight: 500, color: "#ffffff" }}>{p.formResponseNote}</p>
                                    </div>
            ...
            <footer style={{ padding: isMobile ? "32px 20px" : "40px 72px", borderTop: `1px solid ${white(0.06)}` }}>
                <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: isMobile ? "minmax(0,1fr)" : isTablet ? "minmax(0,1fr) minmax(0,1fr)" : "minmax(0,1fr) minmax(0,1fr) minmax(0,1fr)", gap: isMobile ? 32 : 24, alignItems: "start", minWidth: 0 }}>
                    <div style={{ minWidth: 0 }}>
                        <p style={{ fontSize: 14, color: white(OP.label), letterSpacing: 4, margin: "0 0 8px 0", fontWeight: 700, fontFamily: DISPLAY }}>JUST<span style={{ color: white(0.3) }}>.</span></p>
                        <p style={{ fontSize: 12, color: white(OP.label), margin: 0, fontFamily: BODY, lineHeight: 1.6 }}>Paris, France<br />© {new Date().getFullYear()} Just Group</p>
                    </div>
                    <nav aria-label="Liens du pied de page" style={{ minWidth: 0 }}>
                        <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 8 }}>
                            {FOOTER_NAV.map((link) => (
                                <li key={link.label}>
                                    <a href={link.href} style={{ fontSize: 12, fontFamily: BODY, fontWeight: 500, letterSpacing: 1, overflowWrap: "break-word", color: white(0.55), textDecoration: "none", transition: "color 0.2s ease" }} onPointerOver={(e) => { e.currentTarget.style.color = white(0.9) }} onPointerOut={(e) => { e.currentTarget.style.color = white(0.55) }}>{link.label}</a>
                                </li>
                            ))}
                        </ul>
                    </nav>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: isMobile ? "flex-start" : isTablet ? "flex-start" : "flex-end", minWidth: 0 }}>
                        ...
                        {p.footerEmail && (
                            <a href={`mailto:${p.footerEmail}`} style={{ fontSize: 12, fontFamily: BODY, fontWeight: 400, overflowWrap: "break-word", color: white(0.55), textDecoration: "none", transition: "color 0.2s ease" }} onPointerOver={(e) => { e.currentTarget.style.color = white(0.9) }} onPointerOut={(e) => { e.currentTarget.style.color = white(0.55) }}>{p.footerEmail}</a>
                        )}
                    </div>
                </div>
            </footer>
