...
import type { ReactNode, HTMLAttributes, FormEvent, SyntheticEvent } from "react"
...
const OP = { label: 0.55, tag: 0.58, desc: 0.6, heroDesc: 0.62, caption: 0.62, entityDesc: 0.65, quote: 0.75 } as const
...
const S = {
  label: { fontSize: 9, fontWeight: 700, letterSpacing: 6, textTransform: "uppercase" as const, color: white(OP.label), fontFamily: BODY, marginBottom: 14, marginTop: 0 },
  title: { fontFamily: DISPLAY, fontWeight: 800, color: "#fff", lineHeight: 1, letterSpacing: -3, marginTop: 0 },
  pill: { padding: "8px 18px", borderRadius: 100, border: `1px solid ${white(0.08)}`, background: white(0.03), fontFamily: BODY, fontSize: 11, fontWeight: 500, color: white(OP.entityDesc), letterSpacing: 1, textTransform: "uppercase" as const },
  ghost: (size: number): React.CSSProperties => ({ fontFamily: DISPLAY, fontSize: size, fontWeight: 800, color: white(0.03), lineHeight: 1 }),
  overlay: { position: "absolute" as const, inset: 0, pointerEvents: "none" as const },
  coverImg: { width: "100%", height: "100%", objectFit: "cover" as const, display: "block" as const },
...
const PROVIDERS = [
  { match: (h: string) => h.includes("unsplash.com") || h.includes("imgix.net") || h.includes("imagekit.io") || h.includes("storyblok.com"), resize: (url: string, widths: number[]) => { const sep = url.includes("?") ? "&" : "?"; return widths.map((w) => `${url}${sep}w=${w} ${w}w`).join(", ") } },
  { match: (h: string) => h.includes("cloudinary.com"), resize: (url: string, widths: number[]) => widths.map((w) => `${url.replace("/upload/", `/upload/w_${w},c_limit/`)} ${w}w`).join(", ") },
]
function buildSrcSet(url: string): string | undefined {
  try { const hostname = new URL(url, "https://x.com").hostname; const provider = PROVIDERS.find((p) => p.match(hostname)); return provider?.resize(url, SRCSET_WIDTHS) } catch { return undefined }
}
...
type Responsive = { width: number; height: number; mobile: boolean; tablet: boolean; px: number; sectionPad: string }
...
function useResponsive(containerRef: React.RefObject<HTMLDivElement | null>): Responsive {
  const [size, setSize] = useState({ width: 1280, height: 900 })
  useEffect(() => {
    if (typeof window === "undefined") return
    const node = containerRef.current
    if (!node) return
    const update = () => { const rect = node.getBoundingClientRect(); setSize({ width: Math.max(320, Math.round(rect.width || window.innerWidth || 1280)), height: Math.max(1, Math.round(rect.height || window.innerHeight || 900)) }) }
    update()
  ...
  return { width: size.width, height: size.height, mobile, tablet, px, sectionPad: mobile ? "60px 20px" : tablet ? "80px 40px" : "100px 72px" }
}

function gridCols(mobile: boolean, tablet: boolean, desktop: number, tabletCols = 2, mobileCols = 1): string {
  if (mobile) return mobileCols === 1 ? "1fr" : `repeat(${mobileCols}, minmax(0,1fr))`
...
function SectionHeader({ label, title, titleSize = "clamp(28px,4vw,56px)" }: { label: string; title: string; titleSize?: string }) {
  return (
    <Reveal>
      <p style={S.label}>{label}</p>
      <h2 style={{ ...S.title, fontSize: titleSize, marginBottom: 0, margin: 0 }}>{title}</h2>
    </Reveal>
  )
}

function Marquee({ items, speed = 35, fontSize = 72 }: { items: string[]; speed?: number; fontSize?: number }) {
  const idRef = useRef("mq-" + Math.random().toString(36).slice(2, 8))
  ...
  useEffect(() => {
    ...
    el.id = id
    el.textContent = `@keyframes ${id}{0%{transform:translate3d(0,0,0)}100%{transform:translate3d(-33.333%,0,0)}}`
...
function GalleryCard({ img, cardW, cardH }: { img: GalleryImage; cardW: number; cardH: number }) {
  return (
    <figure style={{ width: cardW, minWidth: cardW, maxWidth: cardW, height: cardH, borderRadius: 16, overflow: "hidden", position: "relative", flexShrink: 0, margin: 0 }}>
      <img src={img.src} alt={`${img.caption} — ${img.label}`} draggable={false} loading="lazy" onError={onImgError} style={{ ...S.coverImg, pointerEvents: "none", userSelect: "none" }} />
      ...
      <figcaption style={{ position: "absolute", bottom: 24, left: 24, right: 24 }}>
        ...
        <div style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 18, color: "#fff", letterSpacing: -0.5 }}>{img.label}</div>
      </figcaption>
    </figure>
  )
}
...
function AutoGallery({ images, speed = 45, responsive }: { images: GalleryImage[]; speed?: number; responsive: Responsive }) {
  const idRef = useRef("gal-" + Math.random().toString(36).slice(2, 8))
  const id = idRef.current
  const reduced = useReducedMotion()
  ...
  const measure = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      else { retryRef.current = setTimeout(() => requestAnimationFrame(() => { const w2 = setRef.current?.getBoundingClientRect().width ?? 0; if (w2 > 0) setTrackWidth(w2) }), 200) }
    })
  }, [])
  useEffect(() => {
    if (typeof window === "undefined") return
    ...
    if (setRef.current && ro) ro.observe(setRef.current)
    const imgs = setRef.current?.querySelectorAll("img") ?? []
    const onLoad = () => measure()
    imgs.forEach((img) => { if (!img.complete) img.addEventListener("load", onLoad) })
    return () => { ro?.disconnect(); imgs.forEach((img) => img.removeEventListener("load", onLoad)); if (retryRef.current) clearTimeout(retryRef.current) }
  }, [images, measure])
  useEffect(() => {
    if (typeof document === "undefined" || trackWidth <= 0 || reduced) return
    document.getElementById(id)?.remove()
    const el = document.createElement("style")
    el.id = id
    el.textContent = `@keyframes ${id}{from{transform:translate3d(0,0,0)}to{transform:translate3d(-${trackWidth + gap}px,0,0)}}`
    document.head.appendChild(el)
    return () => { document.getElementById(id)?.remove() }
  }, [id, trackWidth, gap, reduced])
  return (
    <div role="region" aria-label="Galerie photo Just" style={{ width: "100%", minWidth: 0, overflow: "hidden", maskImage: S.maskWide, WebkitMaskImage: S.maskWide }}>
      <div style={{ display: "flex", width: "max-content", willChange: reduced ? "auto" : "transform", animation: trackWidth > 0 && !reduced ? `${id} ${speed}s linear infinite` : "none" }}>
        <div ref={setRef} style={{ display: "flex", gap, flexShrink: 0 }}>
  ...
  return (
    <section ref={ref} aria-label={name} style={{ minHeight: stacked ? "auto" : `${visualHeight}px`, display: "grid", gridTemplateColumns: stacked ? "minmax(0,1fr)" : "minmax(0,1fr) minmax(0,1fr)", alignItems: "stretch", minWidth: 0 }}>
      <div style={{ order: stacked ? 2 : reverse ? 2 : 1, padding: mobile ? "40px 20px" : tablet ? "56px 40px" : "80px 72px", display: "flex", flexDirection: "column", justifyContent: "center", minWidth: 0 }}>
        <motion.div initial={mo(reduced, { opacity: 0, x: reverse ? 40 : -40 })} animate={isVisible ? { opacity: 1, x: 0 } : {}} transition={tr(reduced, 0.8)} style={{ minWidth: 0 }}>
          ...
          <ul style={{ display: "flex", flexWrap: "wrap", gap: 8, listStyle: "none", padding: 0, margin: 0 }} aria-label={`Services ${name}`}>
            {services.map((service, i) => (
              <motion.li key={service} initial={mo(reduced, { opacity: 0, y: 12 })} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={tr(reduced, 0.5, 0.3 + i * 0.06)} style={S.pill}>{service}</motion.li>
            ))}
          </ul>
        </motion.div>
      </div>
      <motion.div style={{ order: stacked ? 1 : reverse ? 1 : 2, height: `${visualHeight}px`, minHeight: `${visualHeight}px`, position: "relative", overflow: "hidden", minWidth: 0 }} initial={mo(reduced, { opacity: 0, scale: 1.05 })} animate={isVisible ? { opacity: 1, scale: 1 } : {}} transition={tr(reduced, 1.2)}>
        <img src={imageUrl} srcSet={buildSrcSet(imageUrl)} sizes={stacked ? "100vw" : "50vw"} alt={`Photo illustrant ${name}`} loading="lazy" onError={onImgError} style={S.coverImg} />
...
function Counter({ value, label, delay = 0 }: { value: string; label: string; delay?: number }) {
  ...
  useEffect(() => {
    if (!isVisible) return
    if (reduced) { setCount(numericValue); return }
    let mounted = true
    const timeout = setTimeout(() => {
      ...
      const tick = (now: number) => {
        ...
        setCount(Math.floor((1 - Math.pow(1 - p, 4)) * numericValue))
        if (p < 1) rafRef.current = requestAnimationFrame(tick)
      }
      rafRef.current = requestAnimationFrame(tick)
    }, delay * 1000)
    return () => { mounted = false; clearTimeout(timeout); if (rafRef.current !== null) cancelAnimationFrame(rafRef.current) }
  }, [isVisible, numericValue, delay, reduced])
  return (
    <div ref={ref} style={{ textAlign: "center", minWidth: 0 }}>
      <div aria-label={`${value} ${label}`} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(40px,5vw,64px)", color: "#fff", lineHeight: 1, marginBottom: 10, letterSpacing: -4, fontVariantNumeric: "tabular-nums", overflowWrap: "break-word" }}>
        <span aria-hidden="true">{isVisible ? count : 0}{suffix}</span>
      </div>
...
export default function JustPage() {
  ...
  const reduced = useReducedMotion()
  const responsive = useResponsive(rootRef)
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setHeroIndex((p) => (p + 1) % heroSlides.length), 5000)
  ...
  const heroRotatingBase: React.CSSProperties = {
  ...
  const rotatingSlot = (fontPx: number): React.CSSProperties => ({
  ...
  useEffect(() => {
    ...
    if (!document.getElementById("just-page-fonts")) {
      const link = document.createElement("link")
      link.id = "just-page-fonts"
      link.rel = "stylesheet"
      link.href = "https://fonts.googleapis.com/css2?family=Syne:wght@300;400;500;600;700;800&family=Outfit:wght@200;300;400;500;600;700&display=swap"
  ...
  return (
    <div ref={rootRef} style={{ width: "100%", minWidth: 0, background: "#000", color: "#c8c8c8", fontFamily: BODY, overflowX: "hidden", WebkitFontSmoothing: "antialiased", position: "relative" }}>
      <style>{`
      ...
      <header>
        <section aria-label="Présentation Just Group" style={{ minHeight: mobile ? "auto" : "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: heroPad, position: "relative", overflow: "hidden" }}>
          ...
          <motion.p initial={mo(reduced, { opacity: 0 })} animate={{ opacity: 1 }} transition={tr(reduced, 1, 0.2)} style={{ ...S.label, fontSize: mobile ? 9 : 10, color: white(OP.tag), marginBottom: mobile ? 28 : 48, display: "flex", alignItems: "center", gap: 12 }}>
            <motion.span aria-hidden="true" animate={mo(reduced, { scale: [1, 1.5, 1], opacity: [0.3, 0.8, 0.3] })} transition={reduced ? {} : { duration: 2.5, repeat: Infinity }} style={{ width: 5, height: 5, borderRadius: "50%", background: "#fff", display: "inline-block", flexShrink: 0 }} />
            ...
            <h1 style={{ margin: 0, padding: 0, minWidth: 0 }}>
              <div style={rotatingSlot(heroFontPx1)}>
                <AnimatePresence mode="wait" initial={false}>
                  <motion.span key={heroSlides[heroIndex].title} initial={mo(reduced, { opacity: 0, y: 40 })} animate={{ opacity: 1, y: 0 }} exit={mo(reduced, { opacity: 0, y: -40 })} transition={tr(reduced, 0.6)} style={{ ...heroRotatingBase, fontSize: `${heroFontPx1}px`, color: "#fff" }}>
                    {heroSlides[heroIndex].title}
                  </motion.span>
                </AnimatePresence>
              </div>
              <motion.span initial={mo(reduced, { opacity: 0, y: 40 })} animate={{ opacity: 1, y: 0 }} transition={tr(reduced, 0.6, 0.2)} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: `${heroFontPx2}px`, lineHeight: heroLineHeight, color: white(0.15), letterSpacing: mobile ? -2 : -4, display: "block", overflowWrap: "break-word", wordBreak: "break-word", maxWidth: "100%" }}>
                c'est bien plus
              </motion.span>
              <div style={rotatingSlot(heroFontPx3)}>
                <AnimatePresence mode="wait" initial={false}>
                  <motion.span key={heroSlides[heroIndex].subtitle} initial={mo(reduced, { opacity: 0, y: 40 })} animate={{ opacity: 1, y: 0 }} exit={mo(reduced, { opacity: 0, y: -40 })} transition={tr(reduced, 0.6)} style={{ ...heroRotatingBase, fontSize: `${heroFontPx3}px`, fontWeight: 300, fontStyle: "italic", color: white(0.12), letterSpacing: mobile ? -1 : -2 }}>
          ...
          <motion.p initial={mo(reduced, { opacity: 0, y: 30 })} animate={{ opacity: 1, y: 0 }} transition={tr(reduced, 0.8, 0.8)} style={{ marginTop: mobile ? 24 : 34, fontSize: mobile ? 15 : 16, lineHeight: 1.9, maxWidth: mobile ? "100%" : 500, color: white(OP.heroDesc), fontWeight: 300, minWidth: 0, overflowWrap: "break-word" }}>
          ...
          <motion.button onClick={() => setHeroPaused((p) => !p)} aria-label={heroPaused ? "Reprendre le défilement" : "Pause le défilement"} initial={mo(reduced, { opacity: 0 })} animate={{ opacity: 1 }} transition={reduced ? {} : { delay: 1.5 }} whileHover={mo(reduced, { scale: 1.1 })} whileTap={mo(reduced, { scale: 0.95 })} style={{ position: "absolute", bottom: mobile ? 20 : 40, right: px, width: 40, height: 40, borderRadius: "50%", border: `1px solid ${white(0.15)}`, background: white(0.03), color: white(0.6), cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, zIndex: 2, flexShrink: 0 }}>
            {heroPaused ? ICONS.play : ICONS.pause}
          </motion.button>
          {!mobile && (
            <motion.div aria-hidden="true" initial={mo(reduced, { opacity: 0 })} animate={{ opacity: 1 }} transition={reduced ? {} : { delay: 1.5 }} style={{ position: "absolute", bottom: 40, left: px, display: "flex", alignItems: "center", gap: 12 }}>
              <motion.div animate={mo(reduced, { y: [0, 8, 0] })} transition={reduced ? {} : { duration: 2, repeat: Infinity, ease: "easeInOut" }} style={{ width: 1, height: 40, background: `linear-gradient(to bottom,${white(0.5)},transparent)` }} />
      ...
      <main id="just-main" style={{ minWidth: 0 }}>
        ...
        <section aria-label="Chiffres clés" style={{ padding: sectionPad, position: "relative" }}>
          <div aria-hidden="true" style={{ ...S.overlay, background: "radial-gradient(ellipse at center,rgba(255,255,255,0.02) 0%,transparent 70%)" }} />
          <Reveal>
            <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: gridCols(mobile, tablet, 4, 2, 2), gap: mobile ? 28 : 48, minWidth: 0 }}>
        ...
        <section aria-label="Manifesto" style={{ padding: sectionPad, display: "flex", justifyContent: "center" }}>
          <Reveal>
            <div style={{ maxWidth: 800, textAlign: "center", minWidth: 0 }}>
              ...
              <motion.div aria-hidden="true" initial={mo(reduced, { width: 0 })} whileInView={mo(reduced, { width: 60 })} viewport={{ once: true }} transition={tr(reduced, 1, 0.3)} style={{ height: 2, width: reduced ? 60 : 0, background: white(0.15), margin: "36px auto 0" }} />
            </div>
          </Reveal>
        </section>
        ...
        <section aria-label="Nos valeurs" style={{ padding: sectionPad }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", minWidth: 0 }}>
            <SectionHeader label="Ce qui nous définit" title="Nos valeurs" titleSize="clamp(30px,4.5vw,58px)" />
            <ul style={{ display: "grid", gridTemplateColumns: gridCols(mobile, tablet, 4), gap: 1, borderRadius: 20, overflow: "hidden", listStyle: "none", padding: 0, margin: "48px 0 0", minWidth: 0 }}>
              {VALUES.map((v, i) => (
                <Reveal key={v.n} delay={i * 0.06}>
                  <motion.li whileHover={mo(reduced, { backgroundColor: white(0.03) })} transition={{ duration: 0.3 }} style={{ background: white(0.01), padding: mobile ? "40px 20px" : "56px 24px", textAlign: "center", cursor: "default", borderTop: `1px solid ${white(0.04)}`, minHeight: mobile ? 180 : 220, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 0 }}>
        ...
        <section className="jm2-contact-root" aria-labelledby={sectionTitleId} aria-describedby={sectionDescId} style={{ padding: mobile ? "80px 20px 100px" : tablet ? "100px 40px 120px" : "110px 72px 140px", borderTop: `1px solid ${white(0.06)}`, position: "relative" }}>
          <div aria-live="polite" aria-atomic="true" style={{ position: "absolute", width: 1, height: 1, padding: 0, margin: -1, overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 }}>{liveMsg}</div>
          <div aria-hidden="true" style={{ ...S.overlay, background: "radial-gradient(ellipse at center top,rgba(255,255,255,0.03) 0%,transparent 65%)" }} />
          <div style={{ maxWidth: 1320, margin: "0 auto", position: "relative", zIndex: 1 }}>
            ...
            <div className="jm2-contact-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(280px,0.42fr)", gap: mobile ? 56 : 88, alignItems: "start" }}>
              <Reveal>
                <div>
                  {status === "success" ? (
                    ...
                    <section>
                      ...
                      <h3 id={sectionTitleId} style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(32px,4vw,48px)", color: "#ffffff", lineHeight: 1, letterSpacing: -2, margin: "0 0 18px" }}>Dites-nous tout.</h3>
                      <p id={sectionDescId} style={{ margin: "0 0 52px", fontFamily: BODY, fontSize: 15, lineHeight: 1.8, color: white(OP.desc), maxWidth: 620 }}>Les champs marqués d'un astérisque sont obligatoires.</p>
              ...
              <Reveal delay={0.15}>
                <aside className="jm2-contact-sticky" aria-label="Informations de contact" style={{ position: "sticky", top: 100, display: "flex", flexDirection: "column", gap: 24 }}>
                  ...
                  <div style={{ padding: mobile ? "24px" : "28px", borderRadius: 20, background: white(0.02), border: `1px solid ${white(0.08)}` }}>
                    <p style={{ ...S.label, marginBottom: 16, color: white(OP.tag) }}>Note</p>
                    <blockquote style={{ margin: 0, fontFamily: DISPLAY, fontSize: mobile ? 22 : 28, fontWeight: 300, lineHeight: 1.5, color: white(OP.quote), fontStyle: "italic", letterSpacing: -0.8 }}>
