...
const { mobile, tablet, px, sectionPad, width } = useResponsive()
  ...
  const parse = useCallback((s: string) => s.split(",").map((x) => x.trim()).filter(Boolean), [])
  ...
  const handleSubmit = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!WEB3FORMS_ACCESS_KEY) { setStatus("error"); setLiveMsg("La clé du formulaire est absente."); return }
