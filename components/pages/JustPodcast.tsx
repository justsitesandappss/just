...
function useResponsive(containerRef: RefObject<HTMLElement | null>): ResponsiveState {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      raf = window.requestAnimationFrame(() => {
        setWidth(containerRef.current?.clientWidth || window.innerWidth || 1440)
      })
    }
    update()
...
function useGlobalStyles() {
  useEffect(() => {
    ...
    // FIX: Google Fonts supprimé — charger Syne et Outfit via next/font/google dans layout.tsx
    // Raison : évite le CLS, self-host automatique, meilleure perf (audit section 7.1)

    if (!document.getElementById("just-podcast-a11y-styles")) {
      const style = document.createElement("style")
      style.id = "just-podcast-a11y-styles"
      // FIX: .innerHTML → .textContent (sécurité, évite XSS potentiel)
      style.textContent = `
        ...
        .just-root a:focus-visible, .just-root button:focus-visible, .just-root input:focus-visible, .just-root textarea:focus-visible { outline: 2px solid #ffffff !important; outline-offset: 4px !important; border-radius: 2px !important; }
...
function Reveal({
  ...
  return (
    <motion.div
      ...
      animate={rm ? { opacity: 1, y: 0 } : inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 36 }}
      transition={{ duration: rm ? 0 : 0.8, delay: rm ? 0 : delay, ease: EASE }}
...
// FIX: <img> natif conservé ici (pas de next/image dans les composants purement client
// sans layout wrapper), mais les attributs width/height sont ajoutés pour éviter le CLS.
// Si ce composant est utilisé dans un contexte Next.js App Router,
// privilégier next/image avec fill + sizes comme dans JustMedia.
function AccessibleImage({ src, alt, style }: { src: string; alt: string; style?: CSSProperties }) {
...
function Marquee({
  ...
  return (
    // FIX: <section> → <div role="region"> — une <section> sans heading associé n'est
    // pas un landmark valide (RGAA 12.6). role="region" + aria-label est correct.
    <div
      role="region"
    ...
    >
      <ul className="just-visually-hidden">
        {items.map((item, i) => (
      ...
      <motion.div
        aria-hidden="true"
        animate={rm ? { x: 0 } : { x: ["0%", "-33.333%"] }}
        transition={rm ? { duration: 0 } : { duration: speed, repeat: Infinity, ease: "linear" }}
        style={{ display: "flex", gap: 32, width: "max-content", maxWidth: "none", alignItems: "center", paddingInline: 8 }}
      >
        {tripled.map((item, i) => (
          <span key={`${item}-${i}`} style={{ display: "inline-flex", alignItems: "center", gap: 32 }}>
            ...
            <span
              aria-hidden="true"
              style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(255,255,255,0.18)", display: "inline-block", flexShrink: 0 }}
            />
          </span>
        ))}
      </motion.div>
...
function Counter({
  ...
  useEffect(() => {
    // FIX: garde typeof window === "undefined" pour SSR safety
    if (!inView || reducedMotion || typeof window === "undefined") return
    let raf = 0
  ...
  const displayValue = reducedMotion ? numVal : inView ? count : 0
  ...
  return (
    <div ref={ref} style={{ textAlign: "center", minHeight: 120, display: "flex", flexDirection: "column", justifyContent: "center", padding: "12px 8px", minWidth: 0 }}>
      <p
        ...
        style={{ fontFamily: BODY, fontWeight: 700, fontSize: "clamp(34px, 4vw, 64px)", color: COLORS.text, lineHeight: 1, margin: "0 0 10px", letterSpacing: -1.5 }}
      >
        {displayValue}{suffix}
      </p>
      <p style={{ fontSize: 12, color: COLORS.textMuted, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", fontFamily: BODY, margin: 0, lineHeight: 1.5 }}>
        {label}
      </p>
    </div>
  )
...
function WaveformBars({
  ...
  const heights = useMemo(
    () => Array.from({ length: count }).map((_, i) => Math.max(10, 16 + Math.sin(i * 0.42) * 18 + ((i * 17) % 13))),
    [count]
  )
  ...
  return (
    <div aria-hidden="true" role="presentation" style={{ display: "flex", alignItems: "center", gap: 3, height, justifyContent: "center", width: "100%", maxWidth: "100%", overflow: "hidden" }}>
      {heights.map((h, i) => (
        <motion.div
          key={i}
          animate={reducedMotion ? { height: h } : { height: [h * 0.5, h, h * 0.7, h * 0.9, h * 0.6] }}
          transition={reducedMotion ? { duration: 0 } : { duration: 1.6 + (i % 5) * 0.12, repeat: Infinity, ease: "easeInOut", delay: i * 0.03 }}
          style={{ width: 3, height: Math.max(8, h), borderRadius: 2, background: color, flexShrink: 0 }}
        />
      ))}
    </div>
  )
...
// FIX: <motion.article> → <motion.li> — ProcessStep est rendu dans une <ol>,
// un <article> enfant direct d'une liste est invalide HTML/ARIA.
function ProcessStep({
  ...
  return (
    <motion.li
      ...
      animate={reducedMotion ? { opacity: 1, y: 0 } : inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 24 }}
      transition={{ duration: reducedMotion ? 0 : 0.7, delay, ease: EASE }}
      style={{ flex: 1, position: "relative", paddingRight: stacked || isLast ? 0 : 32, minWidth: 0, listStyle: "none" }}
    >
      {!isLast && !stacked && (
        <motion.div
          initial={reducedMotion ? false : { scaleX: 0 }}
          animate={reducedMotion ? { scaleX: 1 } : inView ? { scaleX: 1 } : { scaleX: 0 }}
          transition={{ duration: reducedMotion ? 0 : 1, delay: reducedMotion ? 0 : delay + 0.25, ease: EASE }}
          style={{ position: "absolute", top: 16, left: 32, right: 0, height: 1, background: "linear-gradient(to right, rgba(255,255,255,0.24), rgba(255,255,255,0.06))", transformOrigin: "left" }}
          aria-hidden="true"
        />
      )}
      <div
        style={{ width: 32, height: 32, borderRadius: "50%", border: `1px solid ${COLORS.border}`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24, position: "relative", zIndex: 1, background: COLORS.bg, flexShrink: 0 }}
        aria-hidden="true"
      >
        <span style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, color: COLORS.textSoft }}>{number}</span>
      </div>
      <h3 style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 20, color: COLORS.text, letterSpacing: -0.5, margin: "0 0 10px", lineHeight: 1.2 }}>
        {title}
      </h3>
      <p style={{ fontFamily: BODY, fontSize: 14, lineHeight: 1.8, color: COLORS.textBody, fontWeight: 400, margin: 0, maxWidth: stacked ? "100%" : 260 }}>
        {description}
      </p>
    </motion.li>
  )
  ...
  return (
    <div role="region" aria-roledescription="carousel" aria-label={label} onMouseEnter={clearT} onMouseLeave={startT} onFocus={clearT} onBlur={startT} style={{ minWidth: 0 }}>
      <AnimatePresence mode="wait">
        ...
        >
          {/* FIX: aria-live + aria-atomic sur le blockquote pour annoncer les changements AT */}
          <blockquote
            aria-live="polite"
            aria-atomic="true"
            style={{ fontFamily: DISPLAY, fontSize: "clamp(20px, 2.5vw, 32px)", fontWeight: 400, lineHeight: 1.55, color: COLORS.textBody, margin: 0, letterSpacing: -0.5, fontStyle: "italic" }}
          >
          ...
          <figcaption style={{ fontFamily: BODY, fontSize: 12, fontWeight: 600, color: COLORS.textMuted, letterSpacing: 2, textTransform: "uppercase", marginTop: 24, lineHeight: 1.5 }}>
            {activeItem.by}
          </figcaption>
        </motion.figure>
      </AnimatePresence>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 28, alignItems: "center", flexWrap: "wrap" }}>
          {safeItems.map((item, i) => (
            <button
              key={`${item.by}-${i}`}
              type="button"
              // FIX: aria-label enrichi avec le total pour meilleure annonce AT
              aria-label={`Témoignage ${i + 1} sur ${safeItems.length}`}
              aria-pressed={i === current}
              onClick={() => { setCurrent(i); startT() }}
              style={{
                cursor: "pointer",
                border: `1px solid ${i === current ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.16)"}`,
                background: i === current ? "#fff" : "rgba(255,255,255,0.08)",
...
function ContactInputField({
  id, label, name, type = "text", autoComplete, inputMode, placeholder,
  required = false, value, error, hint, onChange,
}: {
  id: string; label: string; name: string; type?: string; autoComplete?: string
  inputMode?: HTMLAttributes<HTMLInputElement>["inputMode"]; placeholder?: string
  required?: boolean; value: string; error?: string; hint?: string; onChange: (v: string) => void
}) {
  ...
  return (
    <div style={{ position: "relative", minWidth: 0 }}>
      <motion.label
        htmlFor={id}
        animate={{ top: active ? 6 : 24, fontSize: active ? 11 : 15, color: error ? COLORS.error : COLORS.text, letterSpacing: active ? 3.5 : 0 }}
        transition={{ duration: 0.22 }}
        style={{ position: "absolute", left: 0, fontFamily: BODY, fontWeight: active ? 700 : 400, textTransform: active ? "uppercase" : "none", pointerEvents: "none", zIndex: 1, lineHeight: 1 }}
      >
        {label}
        {required && <span aria-hidden="true" style={{ color: COLORS.text }}> *</span>}
      </motion.label>
      ...
      <input
        ...
        style={{ width: "100%", minWidth: 0, background: "transparent", border: "none", borderBottom: `1px solid ${error ? COLORS.error : focused ? COLORS.text : COLORS.border}`, padding: "30px 0 16px", fontFamily: BODY, fontSize: 17, color: COLORS.text, outline: "none", transition: "border-color 0.25s ease", letterSpacing: 0.2 }}
      />
...
function ContactTextareaField({
  id, label, name, placeholder, required = false, value, error, onChange,
}: {
  id: string; label: string; name: string; placeholder?: string
  required?: boolean; value: string; error?: string; onChange: (v: string) => void
}) {
  const [focused, setFocused] = useState(false)
  const active = focused || value.length > 0
  ...
  return (
    <div style={{ position: "relative", minWidth: 0 }}>
      ...
      <textarea
        ...
        onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
        rows={5}
        style={{ width: "100%", minWidth: 0, background: "transparent", border: "none", borderBottom: `1px solid ${error ? COLORS.error : focused ? COLORS.text : COLORS.border}`, padding: "30px 0 16px", fontFamily: BODY, fontSize: 17, color: COLORS.text, outline: "none", resize: "vertical", minHeight: 130, transition: "border-color 0.25s ease" }}
      />
      {error && <p id={`${id}-error`} role="alert" style={{ margin: "10px 0 0", fontFamily: BODY, fontSize: 13, color: COLORS.error }}>{error}</p>}
    </div>
  )
...
function ContactPillSelect({ label, name, options, value, onChange }: {
  ...
  return (
    <fieldset style={{ border: "none", padding: 0, margin: 0, minWidth: 0 }}>
      <legend id={gid} style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: COLORS.text, marginBottom: 18 }}>
        {label}
      </legend>
        ...
        {options.map((opt) => {
          ...
          return (
            <motion.button
              ...
              animate={{ background: active ? "#FFFFFF" : "transparent", borderColor: active ? "#FFFFFF" : "rgba(255,255,255,0.18)", color: active ? "#000000" : "#FFFFFF" }}
              transition={{ duration: 0.2 }}
              // FIX: outline:none supprimé — focus-visible géré par le CSS global .just-root
              style={{ padding: "11px 20px", borderRadius: 100, border: "1px solid", fontFamily: BODY, fontSize: 13, fontWeight: 600, letterSpacing: 1.2, textTransform: "uppercase", cursor: "pointer" }}
            >
              {opt}
            </motion.button>
          )
...
function BookingCalendar({ selectedDate, selectedSlot, onSelectDate, onSelectSlot, mobile = false }: {
  selectedDate: string; selectedSlot: string
  onSelectDate: (d: string) => void; onSelectSlot: (s: string) => void; mobile?: boolean
}) {
  const today = useMemo(() => { const d = new Date(); d.setHours(0, 0, 0, 0); return d }, [])
  const [viewMonth, setViewMonth] = useState(() => new Date(today.getFullYear(), today.getMonth(), 1))

  // FIX: formatDateKey déplacé dans useCallback pour éviter recréation à chaque render
  ...
  return (
    <div style={{ minWidth: 0 }}>
      <fieldset style={{ border: "none", padding: 0, margin: 0, minWidth: 0 }}>
        <legend style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: COLORS.text, marginBottom: 20 }}>
          Réserver un créneau
        </legend>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <button
            ...
            style={{ width: 36, height: 36, borderRadius: "50%", border: `1px solid ${COLORS.border}`, background: "transparent", color: canGoPrev ? COLORS.text : COLORS.textSubtle, cursor: canGoPrev ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            {/* FIX: flèches unicode → SVG pour meilleure compat lecteurs d'écran */}
            <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M15 18l-6-6 6-6" />
            </svg>
          </button>

          <span style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 700, color: COLORS.text, letterSpacing: -0.5 }}>
          ...
          <button
            ...
            style={{ width: 36, height: 36, borderRadius: "50%", border: `1px solid ${COLORS.border}`, background: "transparent", color: COLORS.text, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* FIX: role="row" + role="columnheader" + aria-label complet sur les en-têtes */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 4 }} role="row">
          {DAYS_FR.map((d, i) => (
            <div
              key={d}
              role="columnheader"
              aria-label={DAYS_FR_LONG[i]}
              style={{ textAlign: "center", fontFamily: BODY, fontSize: 10, fontWeight: 700, color: COLORS.textSubtle, letterSpacing: 1, textTransform: "uppercase", padding: "6px 0" }}
            >
              {d}
            </div>
          ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3 }} role="grid" aria-label="Sélection de date">
          {calendarDays.map((day, i) => {
            // FIX: cellule vide doit avoir role="gridcell" pour grille valide
            if (!day) return <div key={`e-${i}`} role="gridcell" />
            ...
            return (
              // FIX: bouton wrappé dans gridcell pour structure grid ARIA correcte
        ...
        {selectedDate && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: EASE }} style={{ marginTop: 24 }}>
            <p style={{ fontFamily: BODY, fontSize: 11, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 14, marginTop: 0 }}>
              Créneau horaire
            </p>
            {/* FIX: role="group" + aria-label sur le conteneur de créneaux */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }} role="group" aria-label="Sélection du créneau horaire">
              {TIME_SLOTS.map((slot) => {
                const active = selectedSlot === slot
                return (
                  <motion.button
                    key={slot} type="button" aria-pressed={active}
                    // FIX: aria-label enrichi + outline:none supprimé
                    aria-label={`${slot}${active ? " — sélectionné" : ""}`}
                    onClick={() => onSelectSlot(active ? "" : slot)}
                    whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
                    animate={{ background: active ? "#FFFFFF" : "transparent", borderColor: active ? "#FFFFFF" : "rgba(255,255,255,0.16)", color: active ? "#000000" : COLORS.textSoft }}
                    transition={{ duration: 0.2 }}
                    style={{ padding: "10px 18px", borderRadius: 100, border: "1px solid", fontFamily: BODY, fontSize: 13, fontWeight: active ? 700 : 500, letterSpacing: 0.5, cursor: "pointer" }}
                  >
                    {slot}
                  </motion.button>
                )
...
export default function JustPodcastPage() {
  ...
  const responsive = useResponsive(rootRef as RefObject<HTMLElement | null>)
  ...
  // FIX: heroTitleId ajouté pour labelliser la section hero (aria-labelledby manquait sa cible)
  ...
  const validateContactForm = useCallback(() => {
    const errors: ContactErrorState = {}
    if (!contactFormData.name.trim()) errors.name = "Merci d\u2019indiquer votre nom."
    if (!contactFormData.email.trim()) errors.email = "Merci d\u2019indiquer votre adresse email."
    else if (!isValidEmail(contactFormData.email)) errors.email = "L\u2019adresse email semble invalide."
    if (!contactFormData.message.trim()) errors.message = "Merci de préciser votre demande."
    else if (contactFormData.message.trim().length < 10) errors.message = "Votre message est un peu trop court."
    return errors
  }, [contactFormData])
  ...
  const handleContactSubmit = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!WEB3FORMS_ACCESS_KEY) { setContactStatus("error"); setContactLiveMessage("La clé Web3Forms est absente. Vérifiez votre fichier .env.local."); return }
    ...
    try {
      const bookingInfo = contactFormData.bookingDate && contactFormData.bookingSlot
        ? `\n\nCréneau souhaité : ${contactFormData.bookingDate} à ${contactFormData.bookingSlot}`
        : contactFormData.bookingDate ? `\n\nDate souhaitée : ${contactFormData.bookingDate}` : ""
      ...
      fd.append("message", contactFormData.message + bookingInfo)
  ...
  }, [contactFormData, validateContactForm, WEB3FORMS_ACCESS_KEY])
  ...
  return (
    <div
      ref={rootRef}
      className="just-root"
      style={{ width: "100%", maxWidth: "100%", background: COLORS.bg, color: COLORS.textBody, fontFamily: BODY, overflowX: "hidden", WebkitFontSmoothing: "antialiased" }}
    >
      <a href="#contenu-principal" className="just-skip-link">
        Aller au contenu principal
      </a>

      {/*
        FIX: <div aria-labelledby="hero-title"> → <section aria-labelledby={heroTitleId}>
        Une <div> ne peut pas être un landmark. La section hero doit être une <section>
        avec un id dynamique React (useId) pour éviter les collisions SSR/hydration.
        Elle est placée AVANT <main> donc hors du contenu principal skipable — correct.
      */}
      <section
        aria-labelledby={heroTitleId}
        style={{ minHeight: responsive.mobile ? "auto" : "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: heroPadding, position: "relative" }}
      >
        ...
        <motion.p
          ...
          style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: COLORS.textMuted, margin: "0 0 32px", display: "flex", alignItems: "center", gap: 12, lineHeight: 1.5, flexWrap: "wrap", position: "relative", zIndex: 1 }}
        >
          <motion.span
            aria-hidden="true"
            animate={reducedMotion ? { scale: 1, opacity: 0.8 } : { scale: [1, 1.45, 1], opacity: [0.35, 0.9, 0.35] }}
            transition={reducedMotion ? { duration: 0 } : { duration: 2.2, repeat: Infinity }}
            style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff", display: "inline-block", flexShrink: 0 }}
          />
        ...
        <div style={{ maxWidth: 1120, width: "100%", position: "relative", zIndex: 1, minWidth: 0 }}>
          <motion.h1
            ...
            style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: responsive.mobile ? "clamp(38px, 13vw, 56px)" : "clamp(54px, 8vw, 108px)", lineHeight: 0.95, color: COLORS.text, margin: 0, letterSpacing: responsive.mobile ? -2 : -4, maxWidth: "100%" }}
          >
            Votre marque
            <br />
            <span style={{ color: "rgba(255,255,255,0.74)" }}>a une voix.</span>
            <br />
            <span style={{ fontWeight: 400, fontStyle: "italic", color: "rgba(255,255,255,0.55)", letterSpacing: responsive.mobile ? -1 : -2 }}>
              On la fait entendre.
            </span>
          </motion.h1>
        </div>
        ...
        <motion.p
          ...
          style={{ margin: "28px 0 0", fontSize: responsive.mobile ? 15 : 17, lineHeight: 1.85, maxWidth: 620, color: COLORS.textBody, fontWeight: 400, position: "relative", zIndex: 1 }}
        >
          Le podcast est le format le plus intime et le plus engageant du digital.
          Chez Just, on conçoit, produit et distribue des podcasts de marque qui
          créent de la confiance, positionnent votre expertise et engagent vos
          audiences, épisode après épisode.
        </motion.p>
        ...
        {!responsive.mobile && (
          <motion.div
            ...
            aria-hidden="true"
            style={{ position: "absolute", bottom: 40, left: responsive.tablet ? 32 : 56, display: "flex", alignItems: "center", gap: 12 }}
          >
            <motion.div
              animate={reducedMotion ? { y: 0 } : { y: [0, 8, 0] }}
              transition={reducedMotion ? { duration: 0 } : { duration: 2, repeat: Infinity, ease: "easeInOut" }}
              style={{ width: 1, height: 40, background: "linear-gradient(to bottom, rgba(255,255,255,0.45), transparent)" }}
            />
      ...
      <main id="contenu-principal" tabIndex={-1}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: fourCols, gap: 14, alignItems: "stretch" }}>
              {whys.map((item, i) => (
                <Reveal key={item.n} delay={i * 0.05} style={{ height: "100%" }}>
                  <motion.article
                    whileHover={reducedMotion ? undefined : { backgroundColor: COLORS.surfaceHover, borderColor: "rgba(255,255,255,0.16)" }}
                    transition={{ duration: 0.25 }}
                    style={{ background: COLORS.surface, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 20, padding: responsive.mobile ? "30px 20px" : "38px 24px", textAlign: "center", minHeight: responsive.mobile ? 220 : 280, height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 0 }}
                  >
            ...
            <div style={{ display: "grid", gridTemplateColumns: threeCols, gap: 14, alignItems: "stretch" }}>
              {services.map((svc, i) => (
                <Reveal key={`${svc.title}-${i}`} delay={i * 0.04} style={{ height: "100%" }}>
                  <motion.article
                    whileHover={reducedMotion ? undefined : { backgroundColor: COLORS.surfaceHover }}
                    transition={{ duration: 0.25 }}
                    style={{ background: COLORS.surface, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 20, padding: responsive.mobile ? "30px 20px" : "38px 26px", position: "relative", overflow: "hidden", minHeight: responsive.mobile ? 210 : 240, height: "100%", display: "flex", flexDirection: "column", minWidth: 0 }}
                  >
        ...
        <section aria-labelledby="process-title" style={{ padding: sectionPadding }}>
          <div style={{ maxWidth: maxW, margin: "0 auto" }}>
            <Reveal>
              ...
              <h2 id="process-title" style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.5vw, 58px)", color: COLORS.text, lineHeight: 1, letterSpacing: responsive.mobile ? -2 : -3, margin: "0 0 56px" }}>
                Comment on travaille.
              </h2>
            </Reveal>
            {/* FIX: <div> → <ol> pour ProcessStep qui rend des <li> — structure sémantique correcte */}
            <ol style={{ display: "flex", flexDirection: processLayout as "row" | "column", gap: responsive.mobile ? 36 : responsive.tablet ? 28 : 0, minWidth: 0, padding: 0, margin: 0 }}>
        ...
        <section aria-labelledby="manifesto-title" style={{ padding: sectionPadding, display: "flex", justifyContent: "center" }}>
          <Reveal style={{ width: "100%" }}>
            <div style={{ maxWidth: 860, textAlign: "center", margin: "0 auto" }}>
              {/*
                FIX: <p id="manifesto-title"> → <h2 id="manifesto-title">
                aria-labelledby sur la section référençait un <p> — invalide,
                seul un heading peut labelliser une section de manière sémantique.
                Classe visually-hidden optionnelle si le titre ne doit pas être visible.
              */}
              <h2
                id="manifesto-title"
                className="just-visually-hidden"
              >
                Manifesto
              </h2>
              <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: COLORS.textMuted, margin: "0 0 28px" }} aria-hidden="true">
                Manifesto
              </p>
              {/* FIX: guillemets typographiques Unicode directs au lieu de " " (ASCII) */}
        ...
        <section
          className="just-dark-form"
          aria-labelledby={contactSectionTitleId}
          aria-describedby={contactSectionDescId}
          style={{ padding: responsive.mobile ? "80px 20px 100px" : responsive.tablet ? "100px 32px 120px" : "110px 56px 140px", borderTop: `1px solid ${COLORS.borderSoft}`, position: "relative", minWidth: 0 }}
        >
                ...
                <div style={{ minWidth: 0 }}>
                  {contactStatus === "success" ? (
                    <motion.section
                      ...
                      style={{ textAlign: "center", padding: responsive.mobile ? "56px 24px" : "80px 40px", background: COLORS.surface, border: `1px solid ${COLORS.borderSoft}`, borderRadius: 24 }}
                    >
                          ...
                          <div style={{ padding: responsive.mobile ? "28px 20px" : "36px 32px", borderRadius: 20, background: "rgba(255,255,255,0.015)", border: `1px solid ${COLORS.borderSoft}` }}>
                            ...
                            {contactFormData.bookingDate && contactFormData.bookingSlot && (
                              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, ease: EASE }} style={{ marginTop: 20, padding: "14px 18px", borderRadius: 12, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", display: "flex", alignItems: "center", gap: 10 }}>
                                <span aria-hidden="true" style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.success, flexShrink: 0 }} />
                                <span style={{ fontFamily: BODY, fontSize: 13, fontWeight: 500, color: "rgba(255,255,255,0.88)" }}>
                                  Créneau sélectionné : <strong>{contactFormData.bookingDate}</strong> à <strong>{contactFormData.bookingSlot}</strong>
                                </span>
                              </motion.div>
                            )}
                          </div>
                          ...
                          <AnimatePresence>
                            {contactStatus === "error" && (
                              <motion.div role="alert" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} style={{ padding: "16px 22px", borderRadius: 12, background: COLORS.errorBg, border: `1px solid ${COLORS.errorBorder}`, fontFamily: BODY, fontSize: 14, color: COLORS.error, lineHeight: 1.7 }}>
                                ...
                                {contactLiveMessage && contactLiveMessage !== "Une erreur est survenue." ? contactLiveMessage : "Veuillez réessayer ou nous contacter directement."}
                              </motion.div>
                            )}
                          </AnimatePresence>
              ...
              <Reveal delay={0.15}>
                <aside className="just-contact-sticky-dark" aria-label="Informations de contact" style={{ position: "sticky", top: 100, display: "flex", flexDirection: "column", gap: 24, minWidth: 0 }}>
                  ...
                  <div style={{ padding: responsive.mobile ? "24px" : "28px", borderRadius: 20, background: "rgba(255,255,255,0.02)", border: `1px solid ${COLORS.borderSoft}` }}>
                    ...
                    {/* FIX: guillemets typographiques Unicode directs */}
